...
Describe 'Write-GitDefaultsHint (issue #160)' {
    ...
    It 'prints nothing when both files already exist (hand-maintained: Copilot review #161 round 8)' {
        New-Item -ItemType File -Path (Join-Path $script:hintRepo '.gitattributes') | Out-Null
        New-Item -ItemType File -Path (Join-Path $script:hintRepo '.gitignore') | Out-Null
        $out = Write-GitDefaultsHint -RepoRoot $script:hintRepo 6>&1 | Out-String
    ...
    It 'still prompts when Pull-SDLC.ai JUST merge-created .gitignore from upstream on this sync (Copilot review #161 round 7+8)' {
        # File exists now (Pull-SDLC.ai dropped a vanilla copy) but the
        # caller signals it was freshly merge-created, so the hint must
        # still fire.
        New-Item -ItemType File -Path (Join-Path $script:hintRepo '.gitignore') | Out-Null
        $out = Write-GitDefaultsHint -RepoRoot $script:hintRepo -NewlyMergedFiles @('.gitignore') 6>&1 | Out-String
        $out | Should -Match 'No language-aware \.gitattributes/\.gitignore found'
    }

    It 'does NOT prompt when the consumer already has a hand-maintained .gitignore and .gitattributes (round 8)' {
        New-Item -ItemType File -Path (Join-Path $script:hintRepo '.gitattributes') | Out-Null
        New-Item -ItemType File -Path (Join-Path $script:hintRepo '.gitignore') | Out-Null
        # Empty NewlyMergedFiles means Pull-SDLC.ai did not create either
        # file on this sync; they were pre-existing.
        $out = Write-GitDefaultsHint -RepoRoot $script:hintRepo -NewlyMergedFiles @() 6>&1 | Out-String
        $out.Trim() | Should -BeNullOrEmpty
    }
}
