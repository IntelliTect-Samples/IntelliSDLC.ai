...
Describe 'Write-GitDefaultsHint (issue #160)' {
    BeforeEach {
        ...
        $script:marker = "# Generated by Initialize-GitDefaults.ps1 on 2025-01-01T00:00:00Z`n"
    }
    AfterEach {
    ...
    It 'prints the both-missing hint exactly once when neither file exists' {
        ...
        $out | Should -Match 'No language-aware \.gitattributes/\.gitignore found'
    }

    It 'prints the gitattributes-only hint with -IncludeGitignore:$false when only .gitignore was generated (Copilot review #161 round 2)' {
        Set-Content -Path (Join-Path $script:hintRepo '.gitignore') -Value $script:marker -NoNewline
        $out = Write-GitDefaultsHint -RepoRoot $script:hintRepo 6>&1 | Out-String
        $out | Should -Match 'No language-aware \.gitattributes found'
        $out | Should -Match '-IncludeGitignore:\$false'
        $out | Should -Match 'your existing \.gitignore is untouched'
    }

    It 'prints the gitignore-only hint with -IncludeGitattributes:$false when only .gitattributes was generated (Copilot review #161 round 2)' {
        Set-Content -Path (Join-Path $script:hintRepo '.gitattributes') -Value $script:marker -NoNewline
        $out = Write-GitDefaultsHint -RepoRoot $script:hintRepo 6>&1 | Out-String
        $out | Should -Match 'No language-aware \.gitignore found'
    ...
    It 'prints nothing when both files contain the Initialize-GitDefaults marker' {
        Set-Content -Path (Join-Path $script:hintRepo '.gitattributes') -Value $script:marker -NoNewline
        Set-Content -Path (Join-Path $script:hintRepo '.gitignore')     -Value $script:marker -NoNewline
        $out = Write-GitDefaultsHint -RepoRoot $script:hintRepo 6>&1 | Out-String
    ...
    It 'still prompts when .gitignore was merge-created by Pull-SDLC.ai but lacks the generator marker (Copilot review #161 round 7)' {
        # Simulates the fresh-sync scenario: Merge-FileFromUpstream creates
        # .gitignore from $script:MergePaths with vanilla content; the file
        # exists but was NOT produced by Initialize-GitDefaults.ps1.
        Set-Content -Path (Join-Path $script:hintRepo '.gitignore') -Value "node_modules/`n" -NoNewline
        $out = Write-GitDefaultsHint -RepoRoot $script:hintRepo 6>&1 | Out-String
        $out | Should -Match 'No language-aware \.gitattributes/\.gitignore found'
    }
}

Describe 'Tombstone deletion staging (issue #160, Copilot review #161 round 6+7)' {
    BeforeEach {
        $script:tombFx = Join-Path $TestDrive ("tomb-" + [guid]::NewGuid().ToString('N'))
    }

    It 'Invoke-PullSDLC stages and commits the tombstone deletion when the consumer had the legacy .gitattributes.template tracked' {
        # Build an upstream that HAD .gitattributes.template at the anchor
        # commit and removes it at the next commit (the tombstone scenario).
        $fx = New-DiffReplayFixture -Root $script:tombFx `
            -Seed   { '# legacy template' | Out-File -Encoding utf8 .gitattributes.template -NoNewline } `
            -Tweak  { Remove-Item .gitattributes.template }

        # Seed the consumer state file with the anchor so we skip bootstrap.
        Set-SdlcSyncState -RepoRoot $fx.Consumer -Remote 'sdlc.ai' -Ref 'main' -Commit $fx.AnchorSha
        Push-Location $fx.Consumer
        try { git add .sdlc-ai-sync.json; git commit -q -m 'seed state' } finally { Pop-Location }

        # Confirm the consumer started with the legacy file tracked.
        Test-Path (Join-Path $fx.Consumer '.gitattributes.template') | Should -BeTrue

        $rc = Invoke-PullSDLC -RepoRoot $fx.Consumer -RemoteName 'sdlc.ai' -NoFetch
        $rc | Should -Be 0

        # The file is gone from the working tree...
        Test-Path (Join-Path $fx.Consumer '.gitattributes.template') | Should -BeFalse

        # ...and the deletion landed in the sync commit (NOT left dangling
        # as an unstaged change). Copilot review #161 round 6 was that the
        # staging loop skipped tracked-but-deleted paths.
        Push-Location $fx.Consumer
        try {
            $unstaged = (git status --porcelain) -join "`n"
            $unstaged | Should -Not -Match '\.gitattributes\.template'
            $deletedInCommit = git log -1 --diff-filter=D --name-only --pretty=format:
            ($deletedInCommit -join "`n") | Should -Match '\.gitattributes\.template'
        } finally { Pop-Location }
    }
}
