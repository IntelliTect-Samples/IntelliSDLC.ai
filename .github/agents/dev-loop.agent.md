...
**⚠️ Worktree Enforcement Gate:**
Before proceeding to Phase 2 (or making **any** file changes), verify:
- Your current working directory path contains `.worktrees/`
- `git branch --show-current` returns a branch name that is NOT `main`
- If either check fails, **STOP** — create the worktree first. Do NOT proceed.

This gate is **mandatory**. No exceptions. If you find yourself editing files outside
a `.worktrees/` directory, you have skipped this phase — stop immediately and fix it.
...
10. **Complete the Pre-Completion Checklist** — before calling `task_complete`, verify every item in the checklist below. Never call `task_complete` early or skip phases.
...
4. Present a summary to the user using this format:
   ...
   **Branch:** `<branch-name>` | **Worktree:** `.worktrees/<name>`

   ### What was done
   - **Implemented:** <feature summary>
   - **Refactored:** <refactoring summary, or "N/A">
   - **Functional tests:** <tests added, or "Skipped — <reason>">
   - **Product spec:** <what was added/updated, or "No changes">

   ### Quality evidence
   - **Loop iterations:** <N>
   - **Tests:** <X> passed, 0 failed
   - **Dry run:** <count> headlines extracted | [Preview](<file-uri>)
   - **Copilot review:** 0 unresolved threads (final round)
   ```

   **The PR number and issue number MUST appear as the first two lines of the summary.**
   Never omit the PR number — retrieve it from the `gh pr create` output or `gh pr list --head <branch>`.
...
**Before calling `task_complete`, you MUST verify every item below.** If any item fails,
do NOT call `task_complete` — go back and fix it first.

```
[ ] WORKTREE: All file changes were made inside a `.worktrees/` directory (not the main checkout)
[ ] BRANCH: Current branch is NOT `main` (`git branch --show-current`)
[ ] BUILD: `dotnet build --no-restore` (or language equivalent) passes with zero errors
[ ] TESTS: `dotnet test --no-build` (or language equivalent) passes — all tests green
[ ] ISSUE: A GitHub issue exists and is linked to the PR with `Closes #N`
[ ] PR: A pull request has been created and pushed (`gh pr list --head <branch>` returns a result)
[ ] PR NUMBER: The PR number is included in the completion summary (first line)
[ ] CODE REVIEW: Code review (Phase 6) was completed — findings addressed or deferred
[ ] COPILOT REVIEW: Copilot review was requested, all threads resolved, and re-review confirmed clean
[ ] DRY RUN: Dry run executed (or explicitly skipped with documented reason)
[ ] PRODUCT SPEC: `product-spec.md` updated if new behavior was added (or confirmed no update needed)
```

**Failure to complete this checklist is a bug.** The dev loop is not done until every
applicable item is checked. Items may be marked N/A only with explicit justification
(e.g., "Dry run: N/A — this change is docs-only with no runtime behavior").
