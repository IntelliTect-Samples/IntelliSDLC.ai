...
# the substitution-table destination is verified gitignored, not assumed (issue #318)
# Delegates to the zero-dep Node script `substitution-table-gitignore.test.js`.
#
...
BeforeAll {
    ...
    $script:TestJs     = Join-Path $script:ScriptsDir 'har/substitution-table-gitignore.test.js'
}

Describe 'the substitution-table destination is verified gitignored, not assumed (issue #318)' {
    ...
    It 'all behavioral assertions pass' {
        ...
        $exit | Should -Be 0
        ($out -join "`n") | Should -Match 'All substitution-table-gitignore tests passed'
    }
}
