...
# a substitution-table destination is classified by whether git ignores it (issue #318)
# Delegates to the zero-dep Node script `subs-destination.test.js`.
#
...
BeforeAll {
    ...
    $script:TestJs     = Join-Path $script:ScriptsDir 'har/subs-destination.test.js'
}

Describe 'a substitution-table destination is classified by whether git ignores it (issue #318)' {
    ...
    It 'all behavioral assertions pass' {
        ...
        $exit | Should -Be 0
        ($out -join "`n") | Should -Match 'All subs-destination tests passed'
    }
}
