...
- **Don't hide autonomous assumptions** — when you resolve an ambiguity by your own assumption because the user is unavailable (running autonomously), list every such assumption (what you assumed + the decision it drove) in the mandatory **Assumptions** field of your final `task_complete` summary (and mirror it in the PR description) for human review, or state "None". See "Surfacing Assumptions -- Report What You Decided Without Me" in `.github/copilot-instructions.md`.
