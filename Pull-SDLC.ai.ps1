...
function Write-GitDefaultsHint {
    ...
    # File-presence alone is misleading because Pull-SDLC.ai sync may have
    # merge-created .gitignore from $script:MergePaths before this hint
    # runs. The honest signal is whether the file was produced by
    # Initialize-GitDefaults.ps1 (which stamps the "Generated by" header).
    # Copilot review #161 round 7.
    $marker = '# Generated by Initialize-GitDefaults.ps1'
    $needsAttrs  = -not (Test-GitDefaultsGeneratedMarker -Path (Join-Path $RepoRoot '.gitattributes') -Marker $marker)
    $needsIgnore = -not (Test-GitDefaultsGeneratedMarker -Path (Join-Path $RepoRoot '.gitignore')     -Marker $marker)
...
function Test-GitDefaultsGeneratedMarker {
    <#
    .SYNOPSIS
        Returns $true iff the file at -Path exists AND its first few KB
        contain the Initialize-GitDefaults.ps1 "Generated by" header.
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
    ...
    try {
        # Read only the first 4 KB; the header lives in the first ~10 lines.
