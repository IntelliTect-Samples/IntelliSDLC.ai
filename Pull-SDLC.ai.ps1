...
function Write-NextStepsBanner {
    ...
    Write-GitDefaultsHint -RepoRoot $RepoRoot -NewlyMergedFiles $NewlyMergedFiles
}
...
function Write-GitDefaultsHint {
    ...
    param(
        [Parameter(Mandatory)][string]$RepoRoot,
        # Files that Pull-SDLC.ai's union-merge step JUST created from
        # scratch on this sync (i.e. Pull-SDLC.ai dropped a vanilla copy
        # in place, e.g. .gitignore from $script:MergePaths). Even though
        # the file now exists on disk, the consumer never explicitly chose
        # a language-aware ruleset, so we should still prompt for
        # Initialize-GitDefaults.ps1. Pre-existing hand-maintained files
        # do NOT appear here and so do NOT trigger the hint. Copilot
        # review #161 round 8.
        [string[]]$NewlyMergedFiles = @()
    )
    $attrsPath = Join-Path $RepoRoot '.gitattributes'
    $ignorePath = Join-Path $RepoRoot '.gitignore'
    $needsAttrs  = (-not (Test-Path -LiteralPath $attrsPath))  -or ($NewlyMergedFiles -contains '.gitattributes')
    $needsIgnore = (-not (Test-Path -LiteralPath $ignorePath)) -or ($NewlyMergedFiles -contains '.gitignore')
    if (-not ($needsAttrs -or $needsIgnore)) { return }
    if ($needsAttrs -and $needsIgnore) {
        Write-Host 'No language-aware .gitattributes/.gitignore found. Run ./Initialize-GitDefaults.ps1 -Language <stack> -Force to generate both (uses alexkaratarakis/gitattributes + github/gitignore at pinned SHAs).' -ForegroundColor Yellow
    }
    elseif ($needsAttrs) {
        Write-Host 'No language-aware .gitattributes found. Run ./Initialize-GitDefaults.ps1 -Language <stack> -IncludeGitignore:$false -Force (your existing .gitignore is untouched).' -ForegroundColor Yellow
    }
    else {
        Write-Host 'No language-aware .gitignore found. Run ./Initialize-GitDefaults.ps1 -Language <stack> -IncludeGitattributes:$false -Force (your existing .gitattributes is untouched).' -ForegroundColor Yellow
    }
}
...
function Invoke-PullSDLC {
    ...
    # Capture pre-merge presence so we can tell Write-GitDefaultsHint
    # which merge-managed files Pull-SDLC.ai created from scratch on
    # this sync. Existing hand-maintained files must NOT trigger the
    # hint, but a freshly merge-created vanilla file should still prompt
    # the consumer to generate the language-aware version. Copilot
    # review #161 round 8.
    $mergePathPreExist = @{}
    foreach ($mp in $script:MergePaths) {
        $mergePathPreExist[$mp] = Test-Path -LiteralPath (Join-Path $RepoRoot $mp)
    }
    ...
    $newlyMergedFiles = New-Object System.Collections.Generic.List[string]
    foreach ($mp in $script:MergePaths) {
        if (Merge-FileFromUpstream -Path $mp -Ref $mergeRef -RepoRoot $RepoRoot) {
            ...
            if (-not $mergePathPreExist[$mp]) {
                $newlyMergedFiles.Add($mp) | Out-Null
            }
        }
    }
    ...
    Write-NextStepsBanner -RepoRoot $RepoRoot -AnchorSource $anchorInfo.Source -ScaffoldedFiles $scaffolded -NewlyMergedFiles $newlyMergedFiles.ToArray()
