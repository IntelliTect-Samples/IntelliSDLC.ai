...
function Invoke-SyncPRMerge {
    <#
    .SYNOPSIS
        Squash-merge an open sync PR into the protected branch, then fast-forward
        the local protected branch and remove the scratch worktree.
    .DESCRIPTION
        Called only in auto-worktree mode when the caller opted into merging
        (via -Confirm in any form, threaded down as -AutoMerge). The merge itself
        is gated by ShouldProcess (ConfirmImpact = 'High'), so:
            -Confirm:$false -> ShouldProcess true, no prompt -> merge
            bare -Confirm   -> prompts, merges on yes
            -WhatIf         -> ShouldProcess false -> reports intent, no merge
        A blocked merge (branch protection, failing checks, conflict) leaves the
        PR open and the worktree intact so the user can act manually.
        Returns $true only when the PR was merged and cleanup completed.
    .OUTPUTS
        [bool] $true if merged and cleaned up, otherwise $false.
    #>
    ...
    $absWorktree = if ([System.IO.Path]::IsPathRooted($WorktreePath)) {
        $WorktreePath
    } else {
    ...
    & gh @GhRepoArgs pr merge $PrRef --squash --delete-branch 2>&1 | Write-Information
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "Could not squash-merge the sync PR automatically; it remains open for manual action: $PrUrl"
        return $false
    }
...
function Invoke-AutoWorktreeSync {
    ...
    # PR reference captured inside the worktree block; the optional squash-merge
    # runs after we leave the worktree directory (see the tail of this function).
    $prRefForMerge = $null
    $prUrlForMerge = $null
    Push-Location $RepoRoot
    try {
    ...
    try {
        ...
        if ($existingPr -and $existingPr.Count -gt 0) {
            ...
            $prRefForMerge = "$($existingPr[0].url)"
            $prUrlForMerge = "$($existingPr[0].url)"
        }
        else {
    ...
    # Optionally squash-merge the PR into the protected branch. This runs AFTER
    # leaving the worktree directory: on Windows a process cannot delete its own
    # current directory, and a successful merge removes the scratch worktree.
    # $AutoMerge is set only when the caller passed -Confirm (in any form), so
    # the default (no -Confirm) opens the PR without merging. A blocked merge
    # leaves the PR open for manual action (handled inside Invoke-SyncPRMerge).
    if ($prRefForMerge) {
        if ($AutoMerge) {
            $null = Invoke-SyncPRMerge -RepoRoot $RepoRoot -ProtectedBranch $ProtectedBranch `
                -SyncBranch $SyncBranch -WorktreePath $absWorktree `
                -PrRef $prRefForMerge -PrUrl $prUrlForMerge -GhRepoArgs $ghRepoArgs
        }
        else {
...
function Invoke-PullSDLC {
    ...
    param(
        ...
        [switch]$NoAutoPR,
        [switch]$NoSelfUpdate,
        # When set (auto-worktree mode), consumer-owned starter files are
        # scaffolded into this directory -- the tree the user invoked the
        # command from -- rather than $RepoRoot, which in worktree mode is the
        # throwaway .worktrees/sdlc-sync scratch tree. Template *content* is
        # still sourced from $RepoRoot (where the sync just materialized it).
        [string]$ScaffoldTargetRoot,
        # When set, after opening the sync PR in auto-worktree mode, squash-merge
        # it into the protected branch (gated by ShouldProcess so -Confirm:$false
        # merges without prompting and a bare -Confirm prompts first). The script
        # sets this from whether the caller passed -Confirm at all, so the default
        # (no -Confirm) opens the PR without merging.
        [switch]$AutoMerge
    )
    ...
    if (-not $commitOnMainEffective -and -not $WhatIfPreference) {
        ...
        if (-not $ctx.Allowed) {
            ...
            $syncArgs = @{
                ...
                NoAutoWorktree     = $true
                # Scaffold into the invoking tree (this $RepoRoot), not the
                # scratch worktree the inner sync runs in.
                ScaffoldTargetRoot = $RepoRoot
            }
            $rc = Invoke-AutoWorktreeSync -RepoRoot $RepoRoot -ProtectedBranch $Branch -NoAutoPR:$NoAutoPR -AutoMerge:$AutoMerge -SyncArgs $syncArgs
            if ($rc -eq 0) {
    ...
    if ($WhatIfPreference) {
        # Honest -WhatIf: the op list above shows file changes, but a real run
        # also (a) scaffolds consumer-owned starter files and (b) on the
        # protected branch opens (and optionally squash-merges) a PR. Preview
        # both so the dry run reflects what a real run actually does. This block
        # only reads the working tree -- it writes nothing.
        $scaffoldTarget = if ($ScaffoldTargetRoot) { $ScaffoldTargetRoot } else { $RepoRoot }
        $wouldScaffold = @()
        foreach ($entry in $script:TemplateScaffoldMap.GetEnumerator()) {
            if (-not (Test-Path -LiteralPath (Join-Path $scaffoldTarget $entry.Value))) {
                $wouldScaffold += $entry.Value
            }
        }
        if ($wouldScaffold.Count -gt 0) {
            Write-Information ''
            Write-Information "Would scaffold $($wouldScaffold.Count) consumer-owned file(s) into the invoking directory (left untracked):"
            foreach ($f in $wouldScaffold) { Write-Information "  + $f" }
        }
    ...
    # Only write the sync-state file when something actually changed this run
    # (ops applied or a merge-managed file updated) OR the recorded state is
    # stale (missing, or pointing at a different upstream commit). Rewriting it
    # on an already-current no-op sync would only bump the `syncedAt` timestamp,
    # dirtying the working tree and producing a spurious empty sync commit (and,
    # in auto-worktree mode, an empty PR). Gating the write keeps a no-op run a
    # true no-op.
    ...
    $didWork = ($ops.Count -gt 0) -or ($mergedPaths.Count -gt 0)
    if ($didWork -or -not $stateCurrent) {
    ...
    $scaffolded = @()
    # In auto-worktree mode the scaffold must land in the directory the user
    # invoked the command from ($ScaffoldTargetRoot), not $RepoRoot (the
    # throwaway .worktrees/sdlc-sync scratch tree). Fall back to $RepoRoot for
    # the direct (non-worktree) path, where they are the same directory.
    $scaffoldTarget = if ($ScaffoldTargetRoot) { $ScaffoldTargetRoot } else { $RepoRoot }
    # Scope the upstream-repo detection to the scaffold target rather than the
    # shell's CWD; otherwise tests (and any caller invoked from inside the
    # upstream repo) misclassify a consumer fixture as the upstream repo and
    # skip scaffolding.
    $originUrl = (& git -C $scaffoldTarget remote get-url origin 2>$null)
    if (Test-IsUpstreamRepo -RemoteUrl $originUrl) {
        ...
        # Template content is sourced from $RepoRoot (where the sync just
        # materialized the upstream-managed *.template files and where the
        # $mergeRef object db lives), but written into $scaffoldTarget.
...
# Merge control uses the standard -Confirm mechanism: passing -Confirm in any
# form (including -Confirm:$false) signals the user wants the sync PR merged.
# -Confirm:$false skips the prompt and auto-squash-merges; bare -Confirm prompts;
# omitting -Confirm opens the PR without merging.
$invokeArgs.AutoMerge = $PSBoundParameters.ContainsKey('Confirm')
